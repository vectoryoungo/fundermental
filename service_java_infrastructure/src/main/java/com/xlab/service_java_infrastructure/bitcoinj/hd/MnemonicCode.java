...
import com.xlab.service_java_infrastructure.bitcoinj.exception.MnemonicException;
...
public class MnemonicCode {
    ...
    /**
     * 将助记词转换成种子（参数passphrase建议不传,以适配各家供应商的钱包）
     * @param words
     * @param passphrase
     * @return
     */
        ...
        public int checkTheWords(List<String> words){
            if (words.size() % 3 > 0) {
                return -3;
            }
            ...
            for (String word : words) {
                // Find the words index in the wordlist.
                ndx = this.wordList.getIndex(word);
                if (ndx < 0) {
                    return -1;
                }
            }
