 ...
 * module: fundermental
 * file: IPLOSTW10.java
 * date: 3/15/19 5:57 PM
 * author: VectorJu
 */

/**
 * @create 2019-03-15 17:57
 * @desc IPLOSTW10
 **/
package com.xlab.service_java_infrastructure.cryptofficial;
...
public class IPLOSTW10 {
    ...
    public IPLOSTW10Parameters createParameters(int n) {
        return new IPLOSTW10ParametersGenerator().init(PairingFactory.getPairingParameters("params/curves/a.properties"), n).generateParameters();
    }
   ...
   public CipherParameters keyGen(CipherParameters privateKey, Element[] y) {
       ...
       keyGen.init(new IPLOSTW10SecretKeyGenerationParameters((IPLOSTW10MasterSecretKeyParameters) privateKey, y));
  ...
  public static void main(String[] args) {
      ...
      if (true == iplostw10.test(iplostw10.keyGen(keyPair.getPrivate(), vectors[1]),
              iplostw10.encrypt(keyPair.getPublic(), vectors[0]))){
          System.out.println(" equals ");
      }

      /*assertEquals(true, iplostw10.test(iplostw10.keyGen(keyPair.getPrivate(), vectors[1]),
                                      iplostw10.encrypt(keyPair.getPublic(), vectors[0])));*/

      // Test for non-orthogonal vectors
      vectors = iplostw10.createNonOrthogonalVectors(keyPair.getPublic(), n);

      if (false == iplostw10.test(
              iplostw10.keyGen(keyPair.getPrivate(), vectors[1]),
              iplostw10.encrypt(keyPair.getPublic(), vectors[0]))) {
          System.out.println(" not equals");
      }
      /*assertEquals(false, iplostw10.test(
                                      iplostw10.keyGen(keyPair.getPrivate(), vectors[1]),
                                      iplostw10.encrypt(keyPair.getPublic(), vectors[0])));*/
  }
